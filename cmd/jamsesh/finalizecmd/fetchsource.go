...
type fetchSource struct {
	...
	URL     string // local path OR https URL with embedded x-access-token
	cleanup func() error
}
...
//   - response carries a pre-composed `remote_url` with the token in the
//     userinfo segment (mirrors the form sessioncmd.buildCloneURL produces
//     for the long-lived clone case)
...
// HTTPS we fetch from the pre-registered jamsesh remote.
func performFetch(out io.Writer, fs *fetchSource) error {
	switch fs.Kind {
	case "local":
		return runGitVerbose(out, "fetch", fs.URL)
	case "https":
		return runGitVerbose(out, "fetch", jamseshRemoteName)
	default:
