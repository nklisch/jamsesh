...
func (q *Queries) ListExpiredPlaygroundSessions(ctx context.Context, arg ListExpiredPlaygroundSessionsParams) ([]Session, error) {
	rows, err := q.db.QueryContext(ctx, listExpiredPlaygroundSessions, arg.OrgID, arg.HardCapAt, arg.IdleTimeoutAt)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (q *Queries) PurgeExpiredTombstones(ctx context.Context, expiresAt time.Time) error {
	_, err := q.db.ExecContext(ctx, purgeExpiredTombstones, expiresAt)
	return err
}
