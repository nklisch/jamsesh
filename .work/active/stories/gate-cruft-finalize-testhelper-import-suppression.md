...
Final Opus review found the earlier consolidated implementation note was too broad. Corrected in the follow-up pass by deleting the obsolete `finalize.FinalizeLockTTL` import keeper from `internal/portal/finalize/testhelpers_test.go`; the file already uses `finalize.Handler` and `finalize.New`.
