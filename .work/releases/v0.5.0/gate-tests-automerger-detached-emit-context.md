...
Final Opus review found the missing test had not been added. Corrected in the follow-up pass with `TestApply_EmitRetry_DetachedContextEmitsAfterWorkerCancel` in `internal/portal/automerger/emit_retry_test.go`, which cancels the worker context before `Apply` and asserts the detached emit path still records `merge.succeeded`.
